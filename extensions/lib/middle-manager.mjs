...
function mmKickoff(favorites) {
  ...
  return [
    ...
    `2. Run board_list_tasks (you have all-groups visibility) to see every project's tasks. Focus only on tasks whose group matches one of the managed projects listed above.`,
    "3. For each managed task, review its activity log and comments (board_get_task), and the liveness of its assignee (from mail_list_agents):",
    "   - Stuck / idle worker: mail them a nudge asking for a progress update (mail_send), or post a board comment.",
    "   - Dead assignee (not connected, task stalled): reassign to a live same-group worker if one exists, or move the task back to To Do / Backlog and note why in a board comment. Do not leave it stuck on a dead worker.",
    "   - Flagged unclear: read the worker's questions in the activity log. If you can resolve it (refine the spec, reassign, find the answer), do so and clear the flag with board_flag_task clear:true. If you genuinely can't resolve it, leave it flagged and mention it in your summary to human.",
    "   - Finished work still sitting in In Progress / Review: move it to Done (board_move_task).",
    "   - Done tasks that should leave the active board: move them to 'archive' (board_move_task column:'archive').",
    "4. Curate the managed-projects list: use mail_set_project_favorite to add a project that clearly needs ongoing oversight, or remove (unfavorite) one whose work is fully done and archived. Be conservative — only unfavorite when there's genuinely nothing left to manage.",
