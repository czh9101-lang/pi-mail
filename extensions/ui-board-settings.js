...
function boardSettingsCard(board) {
  ...
  // ── Debug Console (console error viewer) ──────────────────────────────
  card.appendChild(el("h3", null, "Debug Console"));
  card.appendChild(el("p", "muted", "Recent client-side errors and warnings captured from agents, the web UI, and connected mobile apps. Use the get_console_errors tool to query these from the AI assistant. Entries are held in a 100-entry ring buffer."));
  const dcWrap = el("div"); dcWrap.style.maxHeight = "280px"; dcWrap.style.overflowY = "auto";
  dcWrap.style.background = "var(--bg-subtle)"; dcWrap.style.border = "1px solid var(--border)";
  dcWrap.style.borderRadius = "6px"; dcWrap.style.padding = "8px"; dcWrap.style.fontFamily = "monospace"; dcWrap.style.fontSize = "12px";
  dcWrap.style.whiteSpace = "pre-wrap"; dcWrap.style.wordBreak = "break-all";
  dcWrap.id = "debug-console";
  dcWrap.textContent = "Loading…";
  // Fetch errors on mount (async)
  (async () => {
    try {
      const r = await (await fetch("/api/console-errors?limit=50", { cache: "no-store" })).json();
      dcWrap.innerHTML = "";
      if (!r.entries || !r.entries.length) {
        dcWrap.appendChild(el("div", "empty", "No console errors captured."));
        return;
      }
      for (const e of r.entries) {
        const row = el("div");
        row.style.padding = "2px 0";
        row.style.borderBottom = "1px solid var(--border-faint)";
        const time = new Date(e.timestamp).toLocaleTimeString();
        const lvl = e.level === "warn" ? "⚠" : "❌";
        const src = e.source ? ` [${e.source}]` : "";
        row.textContent = `${time} ${lvl}${src} ${e.message}`;
        if (e.stack) {
          const stk = el("div");
          stk.style.color = "var(--dim)";
          stk.style.paddingLeft = "16px";
          stk.style.whiteSpace = "pre-wrap";
          stk.textContent = e.stack.split("\n").slice(0, 3).join("\n");
          row.appendChild(stk);
        }
        dcWrap.appendChild(row);
      }
    } catch { dcWrap.textContent = "Failed to load console errors."; }
  })();
  const dcBtns = el("div"); dcBtns.style.marginTop = "6px"; dcBtns.style.display = "flex"; dcBtns.style.gap = "6px";
  const dcRefresh = el("button", "btn secondary mini", "↻ Refresh");
  dcRefresh.addEventListener("click", async () => {
    dcWrap.innerHTML = ""; dcWrap.textContent = "Loading…";
    try {
      const r = await (await fetch("/api/console-errors?limit=50", { cache: "no-store" })).json();
      dcWrap.innerHTML = "";
      if (!r.entries || !r.entries.length) { dcWrap.appendChild(el("div", "empty", "No console errors captured.")); return; }
      for (const e of r.entries) {
        const row = el("div");
        row.style.padding = "2px 0";
        row.style.borderBottom = "1px solid var(--border-faint)";
  ...
  const dcClear = el("button", "btn secondary mini", "Clear");
  dcClear.addEventListener("click", async () => {
    try { await fetch("/api/console-errors", { method: "DELETE", cache: "no-store" }); }
    catch {}
    dcWrap.innerHTML = "";
    dcWrap.appendChild(el("div", "empty", "No console errors captured."));
  });
  dcBtns.appendChild(dcRefresh); dcBtns.appendChild(dcClear);
  card.appendChild(dcWrap);
  card.appendChild(dcBtns);
