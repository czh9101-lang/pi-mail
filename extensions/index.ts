...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    description:
      "Retrieve recent console errors and warnings captured from clients (mobile app, web UI, agents). " +
      "Returns entries newest-first from a ring buffer of the last 100 entries.",
    promptSnippet: "Retrieve recent console errors",
    parameters: Type.Object({
      limit: Type.Optional(Type.Number({ description: "Max entries to return (default: all matching)" })),
      level: Type.Optional(
        Type.String({ description: "Filter by level: 'error', 'warn', or 'all' (default: 'all')" })
      ),
    }),
    async execute(_id, params, _signal, _onUpdate, _ctx) {
      if (!st.connected || !st.client) {
        return { content: [{ type: "text" as const, text: "❌ Not connected to mail daemon" }] };
      }
      try {
        const resp = await st.client.request<{
          type: string;
          entries?: Array<{ timestamp: number; level: string; message: string; stack?: string; source?: string }>;
          message?: string;
        }>(
          { type: "console_errors_get", limit: params.limit, level: params.level },
          10_000
        );
        if (resp.type === "error") {
        ...
        if (!entries.length) {
          return { content: [{ type: "text" as const, text: "✅ No console errors captured." }] };
        }
        const lines = entries.map((e) => {
          ...
          const stack = e.stack ? `\n  ↳ ${e.stack.split("\n").slice(0, 3).join("\n  ↳ ")}` : "";
          return `[${time}] ${lvl}${src}: ${e.message}${stack}`;
        });
        return {
          content: [
            {
              type: "text" as const,
              text: `📋 Console errors — ${entries.length} entry(s)\n\n${lines.join("\n")}`,
            },
          ],
          details: { entries },
        };
